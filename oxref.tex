...
%
% We patch the truncated date formats from `biblatex2.sty' to use the
% above function.
%
...
% calculation commands defined in `biblatex2.sty' for use by
% `\printdate'.
%
\renewrobustcmd*{\mkbibrangefull}[2]{%
  \iffieldundef{#2year}{%
    \iffieldundef{#2month}{}{%
      \printtext[#2date]{%
...
\renewrobustcmd*{\mkbibrangefullextra}[2]{%
  \iffieldundef{#2year}{%
    \iffieldundef{#2month}{}{%
      \printtext[#2date]{%
        \csuse{mkbibdate#1}{}{#2month}{#2day}%
...
%
% We declare a variant of the `last-first' name format that uses the
% corresponding shortname if available, with the long version in
% parentheses.
% TODO: suppress display of the long version if the same as the short
% version.
%
\DeclareNameFormat{oxyear-last-first}{%
  \ifthenelse{
    \( \ifcurrentname{author} \and \not \ifnameundef{shortauthor} \)
    \or
    \( \ifcurrentname{editor} \and \not \ifnameundef{shorteditor} \)
  }{%
    \edef\bbx@ox@currname{\currentname}
    \printnames[\currentname][\value{listcount}-\value{listcount}]{short\bbx@ox@currname}%
...
\DeclareNameAlias{author}{oxyear-last-first}
\DeclareNameAlias{editor}{oxyear-last-first}
...
\todoc[oxnotes]|\PassOptionsToPackage{style=oxnotes}{biblatex}|
...
\begin{itemize}
  \item ritter1838hap: match up lists of locations and publishers.
  \item beckford1823v: allow subtypes of more entries to be printed.
  \item wp2013oup, rubinstein1956rpc: fix implementation of date type.
  \item perry2013ifm: find way of expressing podcast publisher.
...
|@proceedings{iau1922tia,
  ...
  sortname = {{International Astronomical Union}},
...
|@proceedings{iced2009dik,
  ...
  sortkey = {Proceedings of the 17th International Conference on Engineering Design (ICED ’09)},
