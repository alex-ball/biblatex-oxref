...
\usepackage{xpatch,url-apa,csquotes,xcolor,xparse,multicol,fontawesome}
...
\newcommand{\fa}[1]{\makebox[1em][c]{\faicon{#1}}}
...
\end{writeverbatim}
\addtostream{src}{\space\space\space\space[\Version\space Footnote-based biblatex style inspired by the Oxford Guide to Style]}
\begin{writeverbatim}{src}
...
\end{writeverbatim}
\addtostream{src}{\space\space\space\space[\Version\space Author-year biblatex style inspired by the Oxford Guide to Style]}
\begin{writeverbatim}{src}
...
% #1 = target reference text for oxnotes
% #2 = target reference text for oxyear
% #3 = bib record
\makeatletter
\NewDocumentCommand{\bibexample}{mm+v}{%
  \addtostream{src}{#3}
  \StrBetween*[1,1]{#3}{\@charlb}{,}[\bibkey]
  \IfValueT{#1}{%
    \docbibexample{notesdoc}{#1}{\bibkey}{#3}
  }
  \IfValueT{#2}{%
...
\newcommand{\docbibexample}[4]{%
  ...
  \addtostream{#1}{\space\space\string\fa{book} #2}
  \addtostream{#1}{\space\space\string\par\string\fa{cog} \string\fullcite{#3}}
...
\documentclass[extrafontsizes,11pt,a4paper,oneside]{memoir}
\PassOptionsToPackage{bibstyle=oxyear,citestyle=authoryear}{biblatex}
\input{oxref-preamble}
