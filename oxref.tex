...
\usepackage{htabular,url-apa,csquotes,xcolor,xparse,multicol}
...
% #2 = page range
% #3 = bib key
\NewDocumentCommand\testrow{mom}{\parbox{\textwidth}{\biburlsetup #1\\%
...
\openoutputfile{oxnotes.bbx}{src}
\begin{writeverbatim}{src}
\ProvidesFile{oxnotes.bbx}
\end{writeverbatim}
\addtostream{notesbbx}{\space\space\space\space[\Version\space Footnote-based biblatex style inspired by the Oxford Guide to Style]}
\begin{writeverbatim}{src}
...
\def\isInferred{inferred} % Where authorship isn't explicit
\def\whenPublished{published} % URL date is when published online

% Standard style is not far off
...
\DeclareNameFormat{entrystartname}{%
  \ifbibliography
  {\ifnumequal{\value{listcount}}{1}
    {\iffirstinits
      {\usebibmacro{name:last-first-sc}{#1}{#4}{#5}{#7}}
      {\usebibmacro{name:last-first-sc}{#1}{#3}{#5}{#7}}%
      \ifblank{#3#5}
      {}
...
\DeclareNameAlias{author}{entrystartname}
\DeclareNameAlias[book]{editor}{entrystartname}
\DeclareNameAlias[booklet]{editor}{entrystartname}
\DeclareNameAlias[collection]{editor}{entrystartname}
\DeclareNameAlias[manual]{editor}{entrystartname}
\DeclareNameAlias[misc]{editor}{entrystartname}
\DeclareNameAlias[mvbook]{editor}{entrystartname}
\DeclareNameAlias[mvcollection]{editor}{entrystartname}
\DeclareNameAlias[mvproceedings]{editor}{entrystartname}
\DeclareNameAlias[mvreference]{editor}{entrystartname}
\DeclareNameAlias[online]{editor}{entrystartname}
\DeclareNameAlias[proceedings]{editor}{entrystartname}
\DeclareNameAlias[reference]{editor}{entrystartname}

\newbibmacro*{name:last-first-sc}[4]{%
  \ifuseprefix
  {\usebibmacro{name:delim}{\textsc{#3}\textsc{#1}}%
    \usebibmacro{name:hook}{\textsc{#3}\textsc{#1}}%
    \ifblank{\textsc{#3}}{}{%
      \ifcapital
      {\mkbibnameprefix{\MakeCapital{\textsc{#3}}}\isdot}
      {\mkbibnameprefix{\textsc{#3}}\isdot}%
      \ifpunctmark{'}{}{\bibnamedelimc}}%
    \mkbibnamelast{\textsc{#1}}\isdot
    \ifblank{\textsc{#4}}{}{\bibnamedelimd\mkbibnameaffix{\textsc{#4}}\isdot}%
    \ifblank{\textsc{#2}}{}{\addcomma\bibnamedelimd\mkbibnamefirst{\textsc{#2}}\isdot}}
  {\usebibmacro{name:delim}{\textsc{#1}}%
    \usebibmacro{name:hook}{\textsc{#1}}%
    \mkbibnamelast{\textsc{#1}}\isdot
    \ifblank{\textsc{#4}}{}{\bibnamedelimd\mkbibnameaffix{\textsc{#4}}\isdot}%
    \ifblank{\textsc{#2}\textsc{#3}}{}{\addcomma}%
    \ifblank{\textsc{#2}}{}{\bibnamedelimd\mkbibnamefirst{\textsc{#2}}\isdot}%
    \ifblank{\textsc{#3}}{}{\bibnamedelimd\mkbibnameprefix{\textsc{#3}}\isdot}}}
...
\renewbibmacro*{name:first-last}[4]{%
  ...
  \ifblank{#4}{}{\ifnumeral{#4}{}{\addcomma}\bibnamedelimd\mkbibnameaffix{#4}\isdot}}
...
% This command is extracted and adapted from code by Marco Daniel:
% see http://tex.stackexchange.com/questions/23483/
% It compares two years, and outputs a full or truncated version of
% the second year, depending on how similar it is to the first.
...
\newbibmacro*{maintitle+volume}{%
  ...
  {\printtext[publication]{%\bibstring{volume}\addspace
...
\openoutputfile{oxyear.bbx}{src}
\begin{writeverbatim}{src}
\ProvidesFile{oxyear.bbx}
\end{writeverbatim}
\addtostream{yearbbx}{\space\space\space\space[\Version\space Author-year biblatex style inspired by the Oxford Guide to Style]}
\begin{writeverbatim}{src}
...
\def\dateType{}
\DeclareEntryOption{datetype}{\def\dateType{#1\space}}
\DeclareEntryOption{urldatetype}{%
  \renewbibmacro*{url+urldate}{%
    \printtext{#1}\space\printurldate\space
    \printfield{url}%
    \iffieldundef{url}{}{\nopunct}}%
}
\DeclareBibliographyCategory{reviewed}
...
\renewbibmacro*{date}{\printtext{\dateType}\printdate}
...
\DeclareFieldFormat[book,inbook,collection,incollection,proceedings,inproceedings,reference,inreference]{volume}{\Rn{#1}}
...
\end{writeverbatim}
\addtostream{lbx}{\space\space\space\space[\Version\space British English conventions required by the biblatex-oxref styles]}
\begin{writeverbatim}{src}
...
\newoutputstream{notesdoc}
\openoutputfile{oxnotes-doc.tex}{notesdoc}
\newoutputstream{yeardoc}
\openoutputfile{oxyear-doc.tex}{yeardoc}

\usepackage{xstring}
% \bibexample adds a sample reference to the documentation.
% #1 = where to send code: 'notes' or 'year' (default is all)
% #2 = target reference text
% #3 = bib record
\makeatletter
\newcommand{\bibexample}{\bgroup\obeylines\obeyspaces\@bibexample}
\newcommand{\@bibexample}[3][notes,year]{%
  \addtostream{src}{#3}
  \StrBetween*[1,1]{#3}{\@charlb}{,}[\bibkey]
  \IfSubStr{#1}{notes}{%
    \docbibexample{notesdoc}{#2}{\bibkey}{#3}
  }{}
  \IfSubStr{#1}{year}{%
    \docbibexample{yeardoc}{#2}{\bibkey}{#3}
  }{}
  \egroup
}
\makeatother
...
% #3 = bib key
% #4 = bib record
\newcommand{\docbibexample}[4]{%
  \addtostream{#1}{\string\begin{tcolorbox}}
  \addtostream{#1}{\space\space\string\emph{Desired output:}\string\\}
  \addtostream{#1}{\space\space#2}
  \addtostream{#1}{\space\space\string\par\string\emph{Actual output:}\string\\}
  \addtostream{#1}{\space\space\string\fullcite{#3}}
  ...
  \addtostream{#1}{\string\end{tcolorbox}}
  \addtostream{#1}{}
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% DOCUMENTATION FOR OXNOTES
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{writeverbatim}{notesdoc}
\documentclass[extrafontsizes,11pt,a4paper,oneside]{memoir}
...
\begin{description}
  ...
  \\\egcite{eliot1977m}
  \\\egcite{benvenuti1986op}
  \\\egcite{oconor1977sjf}
  
  \item[No publisher]~\par
  Author, \emph{Title} (Location, Year).
  \\\egcite{balfour1768pe}
  
  \item[Edited book]~\par
  Author, \emph{Title}, \lit{ed.} Editor(s) (Location: Publisher, Year). \\\egcite{boethius1976dds}\par
  \emph{Title}, \lit{ed.} Editor(s) (Location: Publisher, Year).
  \\\egcite{boas.botschuyver1952dc}
\end{description}
...
\begin{description}
  ...
  \emph{Title}, \lit{ed.} Editor(s) (Location: Publisher, Year).
  \\\egcite{fortenbaugh.etal1991tes}
\end{description}
...
\begin{description}
  ...
  \\\egcite{denniston1934gp}
  \\\egcite{gibbon1686dfs}
...
\testrow{Mary A. Rouse and Richard H. Rouse, \enquote{The Book Trade at the University of Paris, c.1250--c.1350}, in eid., \emph{Authentic Witnesses: Approaches to Medieval Texts and Manuscripts} (Publications in Medieval Studies, 17; Notre Dame, IN, 1991), 259--338 at 332.}%
[332]{rouse.rouse1991btu}

\section*{15.2.1} % Books: author's name

\testrow{Rosemary Stewart et al. (eds.), \emph{Managing in Britain} (London, 1994).}%
{stewart.etal1994mb}

\testrow{\emph{Theophrastus of Eresus: Sources for his Life, Writings, Thought, and Influence}, ed. William W. Fortenbaugh et al., 2 vols. (Philosophia Antiqua, 54; Leiden, 1991).}%
{fortenbaugh.etal1991tes}

\testrow{Gerald de Vaucouleurs et al., \enquote{The New Martian Nomenclature of the International Astronomical Union}, \emph{Icarus}, 26 (1975), 85--98.}%
{vaucouleurs1975nmn}

\testrow{George Eliot, \emph{Middlemarch} (New York: W. W. Norton, 1977).}%
{eliot1977m}

\testrow{Mark Twain, \emph{A Connecticut Yankee at King Arthur's Court}, with an introduction by Justin Kaplan (Harmondsworth: Penguin, 1971).}%
{twain1971cyk}

\testrow{C. L. Dodgson (Lewis Carroll), \emph{Symbolic Logic} (Oxford: Clarendon Press, 1896).}%
{dodgson1896sl}

\testrow{Pseudo-Boethius, \emph{De disciplina scolarium: Édition critique, introduction et notes}, ed. Olga Weijers (Leiden, 1976).}%
{boethius1976dds}

\testrow{\emph{Distichia Catonis}, ed. Marcus Boas and Henricus Johannes Botschuyver (Amsterdam, 1952).}%
{boas.botschuyver1952dc}

\testrow{{[James Balfour]}, \emph{Philosophical Essays} (Edinburgh, 1768).}%
{balfour1768pe}

\aside{In the \emph{Oxford Style Manual}, there is no comma after ‘rev. edn.’ in the following reference; this may be an error.}

\testrow{{[John Gibbon]}, \emph{Day-Fatality: Or Some Observations on Days Lucky and Unlucky}, (London, 1678; rev.~edn., 1686).}%
{gibbon1686dfs}

\testrow{\emph{Stories after Nature} (London: Allman, 1822).\quad [\enquote{Anonymous} in Bibliography]}%
{anon1822san}

\testrow{\emph{Liber usualis} (Tournai: Desclée, 1956).}%
{anon1956lu}

\section*{15.2.2} % Books: titles and subtitles

\testrow{Thomas A. Brady, Jr., Heiko A. Oberman, and James D. Tracy (eds.), \emph{Handbook of European History, 1400–1600: Late Middle Ages, Renaissance and Reformation}, i: \emph{Structures and Assertions} (Leiden: E. J. Brill, 1994).}%
{brady.etal1994hehsa}

\testrow{Antonia Tissoni Benvenuti, \emph{L'\emph{Orfeo} del Poliziano} (Padua: Editrice Antenore, 1986).}%
{benvenuti1986op}

\testrow{Roderick O'Conor, \emph{A Sentimental Journal through \enquote{Finnegan's Wake}, with a Map of the Liffey} (Dublin: HCE Press, 1977).}%
{oconor1977sjf}

\section*{15.2.3} % Books: editions

\testrow{J. H. Baker, \emph{An Introduction to English Legal History} (3rd edn., 1990), 419--21.}%
[419--421]{baker1990iel}

\testrow{J. D. Denniston, \emph{The Greek Particles} (1934; 2nd edn., Oxford, 1954).}%
{denniston1934gp}

\testrow{Bruno Bettelheim, \emph{The Uses of Enchantment: The Meaning and Importance of Fairy Tales} (Harmondsworth: Penguin Books, 1988) (first pub. 1976).}%
{bettelheim1976uem}

\testrow{\emph{The Oxford Dictionary of Quotations}, 5th edn., ed. Elizabeth Knowles (Oxford, 1999).}%
{knowles1999odq}

\section*{15.2.4} % Books: reprints, reprint editions, and facsimiles

\testrow{C. Adam and D. Tannery (eds.), \emph{Œuvres de Descartes} (Paris: Cerf, 1897--1913; repr. Paris: J. Vrin, CNRS, 1964--76).}%
{adam.tannery1897odd}

\testrow{E. Allen, \emph{A Knack to Know a Knave} (London, 1594; facs. edn., Oxford: Malone Society Reprints, 1963).}%
{allen1594kkk}

\section*{15.2.5} % Books: co-publications and foreign editions

\aside{The following effect was achieved by abusing the \code{origlocation}, \code{origpublisher} and \code{origdate} fields.}

\testrow{L. A. Holford-Strevens, \emph{Aulus Gellius} (London: Duckworth, 1988; Chapel Hill: University of North Carolina Press, 1989).}%
{holfordstrevens1988ag}

\section*{15.2.6} % Books: volumes

\testrow{Edmond Vander Straeten, \emph{La Musique aux Pays-Bas avant le XIX\textsuperscript{e} siècle}, 8 vols. (Brussels, 1867--88), ii. 367--8.}%
[\nopp ii.\ \mkcomprange{367--368}]{straeten1867lmp}

\testrow{Edmond Vander Straeten, \emph{La Musique aux Pays-Bas avant le XIX\textsuperscript{e} siècle}, ii (Brussels, 1867--88), 367--8.}%
[367--368]{straeten1867lmp.ii}

[In the \emph{Oxford Style Manual}, the translator is in parentheses in the following reference, but as this pattern does not occur in any other it may be an error or stylistic variation. The following effect was achieved with \code{volume = \{1--iii\}}, \code{pages = \{iv\}}, and \code{pubstate = \{London: Bohn, 1846\}}.]

\testrow{Heinrich Ritter, \emph{The History of Ancient Philosophy}, trans. Alexander J. W. Morrison, 4 vols., i--iii (Oxford: Talboys, 1838--9), iv (London: Bohn, 1846).}%
{ritter1838hap}

\testrow{A. W. Ward and A. E. Waller (eds.), \emph{The Cambridge History of English Literature}, xii: \emph{The Nineteenth Century} (Cambridge: CUP, 1932), 42--56.}%
[42--56]{ward.waller1932che}

\testrow{David Hackett Fischer, \emph{Albion’s Seed: Four British Folkways in America}, (vol.\ i of \emph{America: A Cultural History}) (New York: Oxford University Press, 1989).}
{fischer1989asf}

\section*{15.2.7} % Books: series title

\testrow{Johannes de Garlandia, \emph{De mensurabili musica}, ed. Erich Reimer, 2 vols. (Beihefte zum Archiv für Musikwissenschaft, 10--11; Wiesbaden, 1972).}%
{garlandia1972dmm}

\section*{15.2.12} % Books: multi-author works

\testrow{\emph{Proceedings of the XIV International Congress of Papyrologists: Oxford, 24--31 July 1974} (London, 1975).}%
{icp1975pic}

\section*{15.2.13} % Books: organizations

\testrow{Government of Botswana, \emph{A Human Drought Relief Programme for Botswana} (Gabarone: Ministry of Local Government and Lands, 1980).}{botswana1980hdr}

\testrow{Penal Affairs Consortium, \emph{An Unsuitable Place for Treatment: Diverting Mentally Disordered Offenders from Custody} (London, 1988).}{pac1988upt}

\section*{15.2.15} % Books: translations

\testrow{Bernhard Bischoff, \emph{Latin Palaeography: Antiquity and the Middle Ages}, trans. Dáibhi Ó Cróinín and David Ganz (Cambridge, 1990).}%
{bischoff1990lpa}

\testrow{Joanat Martorell, \emph{Tirant lo Blanc}, trans. with foreword David H. Rosenthal (London, 1984).}%
{martorell1984tlb}

\section*{15.3} % Chapters and articles in books

\testrow{John Shearman, \enquote{The Vatican Stanze: Functions and Decoration}, in George Holmes (ed.), \emph{Art and Politics in Renaissance Italy: British Academy Lectures} (Oxford: Clarendon Press, 1993), 185--240.}%
{shearman1993vsf}

\testrow{W. B. Todd, \enquote{David Hume: A Preliminary Bibliography}, in id. (ed.), \emph{Hume and the Enlightenment: Essays Presented to Ernest Campbell Mossner} (Edinburgh: Edinburgh University Press, 1974).}%
{todd1974dhp}

\testrow{G. Frege, \enquote{On Sense and Reference}, in id., \emph{Philosophical Writings}, trans. and ed. P. T. Geach and M. Black (Oxford: Blackwell, 1952) (originally pub. 1892).}%
{frege1892osr}

\testrow{G. E. L. Owen, \enquote{Philosophical Invective}, in id., \emph{Logic, Science and Dialectic}, ed. M. Nussbaum (Ithaca, NY: Cornell University Press, 1986), 347--64. From \emph{Oxford Studies in Ancient Philosophy}, 1 (1983), 1--25.}%
{owen1986pi}

% Article in yearbook

\testrow{M. Boyce, \enquote{The Parthian \emph{Gsn} and Iranian Minstrel Tradition}, \emph{Journal of the Royal Asiatic Society}, 1957, 10--45.}%
{boyce1957pgi}

\section*{15.4.1} % Articles in periodicals

\testrow{Halil Inalcik, \enquote{Comments on \enquote{Sultanism}: Max Weber's Typification of the Ottoman Polity}, \emph{Princeton Papers in Near Eastern Studies}, 1 (1992), 49--72.}%
{inalcik1992csm}

\testrow{Robert Goldblatt, \enquote{Diodorean Modality in Minkowski Space-Time}, \emph{Studia Logica}, 39/3 (1973), 219--36.}%
{goldblatt1973dmm}

\testrow{Alan Lee, \enquote{England Haunted by Familiar Failings}, \emph{The Times}, 23 June 1995.}%
{lee1995ehf}

\testrow{\enquote{Who's Excellent Now?}, \emph{Business Week}, 5 Nov. 1984, 76--86.}%
{bw1984wen}

\aside{The following effect was achieved by added \code{\textbackslash nopunct} to the end of the journal title.}

\testrow{B. Lindars, \enquote{Ezechiel and Individual Responsibility}, \emph{VT} 15 (1965), 452--67.}%
{lindars1965eir}

\aside{Here the \code{=} and journal title are given as the \code{titleaddon}, and the issue information as the \code{note}.}

\testrow{C. Bec (ed.), \emph{Italie 1500--1550: Une situation de crise?} = \emph{Annales de l'Université Jean Moulin}, 1975/2 (Langues étrangères, 2; Lyon, 1976), 99--109.}%
{bec1976isc}

\testrow{F. Trisoglio, \emph{Gregorio di Nazianzo in un quarentennio di recherche (1925--1965)} = \emph{Rivista Iasalliana}, 40 (1973).}%
{trisoglio1973gnq}

\testrow{X. Zhentao, K. K. C. Yau, and F. R. Stephenson, \enquote{Astronomical Records on the Shang Dynasty Oracle Bones}, \emph{Archaeoastronomy}, 14 (1989); Supplement to \emph{Journal for the History of Astronomy}, 20, pp. S61--S72.}%
{zhentao.etal1989ars}

\section*{15.4.3} % Articles in periodicals: series

\testrow{J. Barnes, \enquote{Homonymy in Aristotle and Speusippus}, \emph{Classical Quarterly}, \textsc{ns} 21 (1971), 65--80.}%
{barnes1971has}

\section*{15.4.4} % Articles in periodicals: reviews

\testrow{J. Dean, review of Philippe Basiron, \emph{My Early Life} (Bourges, 1994), in \emph{Res facta}, 17 (1995), 56--9.}%
{dean1995rpb}

\testrow{H. D. Jocelyn, \enquote{Probus and Virgil}, review of Maria Luisa Delvigo, \emph{Testo virgiliano e tradizione indiretta} (Pisa, 1987), in \emph{CR}, \textsc{ns} 39 (1989), 27--8.}%
{jocelyn1989pav}

\section*{15.5.1} % Theses and dissertations: terminology

\testrow{Bonnie J. Blackburn, \enquote{The Lupus Problem}, Ph.D. diss. (University of Chicago, 1970).}%
{blackburn1970tlp}

\section*{15.6} % Unpublished books and articles

\testrow{Leofranc Holford-Strevens, \enquote{Humanism and the Language of Music Theory Treatises}, paper given at the 65th Annual Meeting of the American Musicological Society, Kansas City, MO, 4--7 Nov. 1999.}%
{holfordstrevens1999hlm}

\testrow{E. H. Gombrich, \emph{Art and Scholarship}, Inaugural Lecture, University College London, 14 Feb. 1957 (London: H. K. Lewis, 1957).}%
{gombrich1957as}
...
\section*{15.14.1} % Audio recordings

\testrow{Francois Couperin, \emph{Pièces de clavecin: Huit préludes de L'Art de toucher le clavecin. Livre I. Troisième et quatrième ordres}, Huguette Dreyfus (Valois, MB 797, 1970).}%
{couperin1970pdc}

\testrow{Paul Hillier, \emph{Proensa} (ECM Records compact disc ECM 1368, 1989).}%
{hillier1989p}

\testrow{Lightnin' Hopkins, \emph{The Complete Aladdin Recordings} (EMI Blues Series, CDP-7-96843-2) (two-volume compact disc set).}%
{hopkinsNDcar}

\testrow{\emph{The Mirror of Narcissus: Songs by Guillaume de Machaut}, Gothic Voices, dir. Christopher Page (Hyperion compact disc CDA 66087).}
{pageNDmn}

\testrow{C. S. Lewis, \emph{The Lion, the Witch, and the Wardrobe}, read by Sir Michael Hordern (2 audio cassettes, TO1611, 1981).}
{lewis1981lww}
...
\section*{15.15.2} % Electronic data: representation

\testrow{Barbara Quint, \enquote{One Hour to Midnight: \emph{Tasini} Oral Arguments at the Supreme Court}, \emph{Information Today} [online journal], 18/5 (May 2001) $\langle$\url{http://www.infotoday.com/newsbreaks/nb010330-1.htm}$\rangle$, accessed 1 July 2001.}%
{quint2001ohm}

\section*{15.15.3} % Electronic data: forms of reference

\testrow{R. M. Ritter, \enquote{About the Oxford Guide to Style}, \emph{Oxford Editorial} [website], (updated 10 Oct.\ 2001) $\langle$\url{http://www.ritter.org.uk/Oxford_Editorial/AboutOGS.html}$\rangle$}%
{ritter2001aog}

\testrow{William Strunk, Jr., \emph{The Elements of Style} (1st edn., Geneva, NY: W. P. Humphrey, 1918), published online July 1999 $\langle$\url{http://www.bartleby.com/141}$\rangle$}%
{strunk1999tes}
...
\end{document}
\end{writeverbatim}
\closeoutputstream{notesdoc}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% DOCUMENTATION FOR OXYEAR
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{writeverbatim}{yeardoc}
\documentclass[extrafontsizes,11pt,a4paper,oneside]{memoir}
\PassOptionsToPackage{bibstyle=oxnotes,citestyle=verbose}{biblatex}
\input{oxref-preamble}
...
@mvbook{fortenbaugh.etal1991tes,
...
% The page range goes in the citation command
@book{baker1990iel,
  author = {J. H. Baker},
...
% First and later edition, same publisher
...
% First and later edition, different publisher
...
% Later edition by different editor
...
% Need special handling for marking facsimile editions
@book{allen1594kkk,
  author = {E. Allen},
...
% Multiple publishers, same title
...
% This hack is not at all portable. Use with care, or use one of the other layouts.
...
% If not using Biber, you would use
% addendum = {From \fullcite[1-25]{osap1983}}
% instead of the related fields
