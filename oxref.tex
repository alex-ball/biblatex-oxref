...
%
% Journal types are also in square brackets.
%
...
%
% The following is mostly verbatim from `authoryear.bbx'.
%
...
%
% 'sortname' format for author, editor, translator, removed.
%
...
\renewbibmacro*{endrelated}{%
  \usebibmacro*{bbx:savehash}}
%
% This macro for author names includes support for inferred authors.
%
\renewbibmacro*{author}{%
  \ifboolexpr{
    test \ifuseauthor
...
Bibliographical items are given throughout this document, and serve two purposes:
...
\spec{Author, \enquote{Title}, \emph{JournalTitle}, Vol/Number (Year); \lit{Supplement to} \emph{MainJournalTitle} Vol/Number (Year), Pages.}
|
...
\spec{Composer, \emph{Title}, Artist (Series, Number, Year).}
|
...
\todoc|
\spec{Composer, \emph{Title}, Ensemble, \lit{dir.} Director; Credit (Publisher Type Number, Year).}
|
...
\todoc|
\spec{Composer, \emph{Title}, Artist (Publisher Series, Number, Year) (Medium).}
|
...
\todoc|
\spec{Author, \emph{Title}, \lit{read by} Reader (Medium, Number, Year).}
|
...
\todoc|
\spec{Speaker, \emph{Title} (Publisher, Number, Year).}
...
\spec{Author, \enquote{SectionTitle}, \emph{Title} [Medium], (Date) $\langle$URL$\rangle$ Division, \lit{accessed} URLDate.}
|
...
\todoc|
\spec{Author, \enquote{SectionTitle}, \emph{Title} [Medium], (Date) $\langle$\lit{DOI} DOI$\rangle$ Division.}
...
\todoc|
  Currently Oxref does not do this; the work is simply listed under its title.
...
\begin{itemize}
  ...
  \item If a particular page is required the first time a source is cited, it should be given after the full page range:
\end{itemize}
|
