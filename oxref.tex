...
% of the collection, the editor name is replaced with \enquote{id.}.
%
...
% main title are put in a parenthetical block between the volume title
...
% == Proceedings ==
%
% In our version of the `event+venue+date' macro, the venue and date are
% set off with a colon instead of parentheses. A bit of jiggery-pokery 
% is used to make the colon italic if it follows italic text.
%
...
% == Grey literature ==
%
% We delegate the handling of report types and numbers to its own macro,
% in case it gets more complicated.
%
...
% punctuation is therefore suppressed. This is used in the standard
% `thesis' driver, which also works well for us.
%
...
%
% We also provide an enhanced version that also includes `type', `number'
% and `edition', which is used by the `report' driver.
%
...
%
% As if that wasn't enough, we provide a third version that also
% includes `series', but uses the more common `publisher' in place of
...
% \begin{itemize}
%   \item
%   `type' and `number' are moved into the publication information
%   block with `institution', `location' and `date'.
%   \item
%   before `related', there is a full stop instead of a comma.
% \end{itemize}
%
